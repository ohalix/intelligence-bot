...
class FundingIngester:
    ...
    async def ingest(self, since: datetime) -> List[Dict[str, Any]]:
        concurrency = int(self.config.get("ingestion", {}).get("funding_concurrency", 4))
        ...
        results = await asyncio.gather(*[_one(u) for u in FUNDING_FEEDS])
        flattened = [x for sub in results for x in sub]

        logger.info(
            "FundingIngester run: attempted=%s success=%s fail=%s items=%s top_errors=%s",
            stats["attempted"],
            stats["success"],
            stats["fail"],
            stats["items"],
